...
async function testWebSocketClient() {
  ...
  try {
    ...
    const transport = new WebSocketClientTransport(
      new URL('http://localhost:3003/ws'),
      {
        path: '/socket.io',
      },
    );
    ...
    console.log('Connected to WebSocket MCP server successfully!');
    ...
    if (tools.tools.some((tool) => tool.name === 'greet')) {
      ...
      const result = await client.callTool('greet', { name: 'WebSocket User' });
    ...
    console.error('WebSocket test failed with error:', error);
  }
}
