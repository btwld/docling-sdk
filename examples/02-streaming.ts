...
async function streamingExample() {
  ...
  try {
    ...
    // Create a larger test document for streaming demo
    const largeTestContent = Array(1000).fill(`
# Chapter ${Math.random()}
...
- Item 3
  ...
  - Another nested item
    ...
    console.log(`📏 Test document size: ${(testBuffer.length / 1024).toFixed(2)} KB`);

    // 1. Stream content to file (Markdown output)
    ...
    await client.convertToStream(
      ...
      outputStream,
      {
        ...
        do_picture_description: false, // Disable for faster processing
      }
    );
    
    console.log("✅ Markdown streaming completed!");

    // 2. Stream to ZIP file (multiple formats)
  ...
  const client = new Docling({
    ...
    progress: {
      ...
      onProgress: (progress) => {
        ...
        process.stdout.write(`\r📊 ${stage}: [${progressBar}] ${percentage.toFixed(1)}%`);
        
        if (progress.uploadedBytes && progress.totalBytes) {
          const speed = progress.bytesPerSecond ? `${(progress.bytesPerSecond / 1024).toFixed(2)} KB/s` : "N/A";
...
async function memoryEfficientProcessing() {
  ...
  try {
    // Create multiple test documents to simulate batch processing
    const documents = Array.from({ length: 5 }, (_, i) => ({
      filename: `document-${i + 1}.md`,
      content: Buffer.from(`# Document ${i + 1}\n\nContent for document ${i + 1} with some data to process.`),
    }));

    console.log(`📚 Processing ${documents.length} documents with streaming...`);
    ...
    for (const doc of documents) {
      ...
      if (result.success && "document" in result.data && result.data.document.json_content) {
        // Stream the result to file instead of keeping in memory
        await writeFile(outputPath, JSON.stringify(result.data.document.json_content, null, 2));
    ...
    console.error("💥 Memory efficient processing error:", error instanceof Error ? error.message : error);
  }
}
